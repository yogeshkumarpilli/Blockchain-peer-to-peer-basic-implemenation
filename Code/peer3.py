...
def connecttracker(address, port):
    ...
    def send_clientport(sock):
        encode_message = bytes(str(23458), 'utf-8')
...
def connectserver(address, port):
    ...
    def peerconnection(c):
        message = "Connect to " + "peer3" + " successful"
...
def mining():
    def handle():
        ...
        while True:
            while len(tomine) < 3:
                blockchain2.checkwork(tomine, block1.received_transactionlist.get(), "peer2")
...
connectserver('localhost', 23458).start()
block1 = blocks()
loadCheeses()
mining().start()
tracker()
